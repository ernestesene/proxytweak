...
static SSL_CTX *ssl_create_context(enum ssl_method method) {
...
#if (PEER_USE_TLS)
SSL *tls_connect(int fd_remote) {
  SSL *ssl_remote = NULL;
  int err = -1;

  if (ctx_client == NULL) goto ret;
  ...
  if (SSL_set_fd(ssl_remote, fd_remote) != 1) goto ret_err;
